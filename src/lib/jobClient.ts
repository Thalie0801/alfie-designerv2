...
export async function getJobStatus(jobId: string): Promise<JobProgress> {
  ...
  // Retrieve carousels from library_assets
  const carouselSteps = steps.filter(s => s.step_type === 'assemble_carousel' && s.status === 'completed');
  const carouselIds: string[] = [];
  for (const step of carouselSteps) {
    const input = step.output_json as any;
    if (input?.carouselId) carouselIds.push(input.carouselId);
  }

  // Also check gen_slide steps for carousel_id in their input
  // Query library_assets for carousels belonging to this job
  ...
  // Filter to slides that belong to carousels generated by this job's steps
  // We need to match by checking if carousel was created around job time
  // For now, get all carousels and group by carousel_id
  if (carouselSlides && carouselSlides.length > 0) {
    ...
    // We need a way to identify which carousels belong to this job
    // Check job_steps for gen_slide with matching carousel references
    const { data: slideStepsWithInput } = await supabase
      .from('job_steps')
      .select('input_json')
      .eq('job_id', jobId)
      .eq('step_type', 'gen_slide');

    const jobCarouselIds = new Set<string>();
    for (const step of slideStepsWithInput || []) {
      const input = step.input_json as any;
      if (input?.carouselId) jobCarouselIds.add(input.carouselId);
    }

    // Add carousels that belong to this job
    for (const [carouselId, slideUrls] of carouselMap) {
      if (jobCarouselIds.has(carouselId) && slideUrls.length > 0) {
        assets.push({
          type: 'carousel',
