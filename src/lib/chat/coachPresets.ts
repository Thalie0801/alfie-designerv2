...
export function chooseCarouselOutline(
  ...
  const nicheLabel = labelForNiche(niche);

  const builder = pickBySeed(CAROUSEL_STRUCTURES, seed);
  const base = builder({ topic, nicheLabel, slides });

  // Si l’utilisateur demande plus de 5 slides,
  // on laisse le système ajouter des détails en dupliquant les points centraux.
...
export function chooseImageVariant(
  ...
  const variants: ReactNode[] = [
    ...
    (
      <div className="text-sm space-y-1">
        <p>
          <strong>Visuel avant / après :</strong> à gauche la situation actuelle, à
          droite le résultat souhaité sur "{topic}".
        </p>
...
export function chooseVideoVariant(
  input: VariantInput,
  seed: number,
): ReactNode {
  const topic = normalizeTopic(input.topic);
  ...
  const variants: ReactNode[] = [
    (
      <div className="text-sm space-y-1">
        <p>
          <strong>Script 1 (face cam) :</strong> "Tu galères avec {topic} ? Laisse-moi
          te montrer 3 choses que {nicheLabel} oublient tout le temps..."
        </p>
    ...
    (
      <div className="text-sm space-y-1">
        <p>
          <strong>Script 2 (B-roll + texte) :</strong> séquences de ton quotidien +
          texte à l’écran avec 3 tips rapides sur "{topic}".
        </p>
        <p>Ideal pour Reels / TikTok silencieux.</p>
      </div>
    ),
    (
      <div className="text-sm space-y-1">
        <p>
          <strong>Script 3 (avant / après) :</strong> "Voici à quoi ressemble {topic}{" "}
          AVANT / APRÈS quand on applique la bonne méthode..."
        </p>
